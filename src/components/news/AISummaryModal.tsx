      ...
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl w-full max-w-2xl overflow-hidden shadow-2xl flex flex-col max-h-[90vh] text-slate-900 dark:text-slate-100">
        ...
        <div className="p-5 border-b border-slate-200 dark:border-slate-800 flex items-center justify-between bg-slate-50 dark:bg-slate-950/50">
          <div className="flex items-center gap-2.5">
            <div className="w-8 h-8 rounded-lg bg-sky-100 dark:bg-sky-950/80 border border-sky-200 dark:border-sky-800 flex items-center justify-center text-sky-600 dark:text-sky-400">
            ...
            <div>
              ...
              <p className="text-xs text-slate-500 dark:text-slate-400 truncate max-w-md font-sans">{article.title}</p>
            </div>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="p-5 overflow-y-auto space-y-5 flex-1">
          {isLoading ? (
            ...
            <>
              ...
              <div className="flex items-center gap-1 bg-slate-100 dark:bg-slate-950 p-1 rounded-xl border border-slate-200 dark:border-slate-800">
                <button
                  ...
                  className={`flex-1 py-1.5 px-3 text-xs font-semibold rounded-lg transition-all cursor-pointer ${
                    activeTab === 'tldr'
                      ? 'bg-white dark:bg-slate-800 text-sky-700 dark:text-sky-300 shadow-2xs border border-slate-200 dark:border-slate-700'
              ...
              {/* Tab Contents */}
              {activeTab === 'tldr' && (
                <div className="p-4 rounded-xl bg-sky-50 dark:bg-sky-950/40 border border-sky-200 dark:border-sky-800/60 text-slate-800 dark:text-slate-200 text-sm leading-relaxed font-sans">
              ...
              {activeTab === 'points' && (
                <ul className="space-y-2.5">
                  {summary.key_points.map((point, idx) => (
                    <li
                      key={idx}
                      className="flex items-start gap-3 p-3 rounded-xl bg-slate-50 dark:bg-slate-950/60 border border-slate-200 dark:border-slate-800 text-xs sm:text-sm text-slate-800 dark:text-slate-200"
                    >
              ...
              {activeTab === 'analysis' && (
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  ...
                  <div className="p-3.5 rounded-xl bg-slate-50 dark:bg-slate-950/60 border border-slate-200 dark:border-slate-800">
                    <span className="text-[11px] font-medium text-slate-500 uppercase tracking-wider block mb-1 flex items-center gap-1">
                  ...
                  {summary.analysis.key_entities && summary.analysis.key_entities.length > 0 && (
                    <div className="p-3.5 rounded-xl bg-slate-50 dark:bg-slate-950/60 border border-slate-200 dark:border-slate-800 sm:col-span-2">
                      <span className="text-[11px] font-medium text-slate-500 uppercase tracking-wider block mb-2 flex items-center gap-1">
                      ...
                      <div className="flex flex-wrap gap-1.5">
                        {summary.analysis.key_entities.map((entity, i) => (
                          <span
                            key={i}
                            className="px-2.5 py-1 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700/80 rounded-md text-xs text-slate-700 dark:text-slate-300 font-medium"
                          >
                            {entity}
                          </span>
                        ))}
        ...
        <div className="p-4 border-t border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-950/50 flex items-center justify-between">
          <button
            ...
            className="flex items-center gap-2 px-3.5 py-1.5 rounded-xl bg-slate-200 dark:bg-slate-800 hover:bg-slate-300 dark:hover:bg-slate-700 text-slate-800 dark:text-slate-200 text-xs font-semibold border border-slate-300 dark:border-slate-700/80 transition-colors disabled:opacity-50 cursor-pointer"
          >
          ...
          <button
            onClick={onClose}
            className="px-4 py-1.5 bg-sky-600 hover:bg-sky-700 text-white font-semibold text-xs rounded-xl transition-colors cursor-pointer"
          >
            Done
          </button>
        </div>
